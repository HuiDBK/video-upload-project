...
class MainWin(BaseWin):
    ...
    def init_layout(self):
        ...
        layout = [
            ...
            [sg.Text('视频文件'), sg.InputText(key='input_video'), sg.FileBrowse('选择', key='video')],
            [sg.Text('字幕文件'), sg.InputText(key='input_srt'), sg.FileBrowse('选择', key='srt')],
            [
                sg.Text('视频大类'),
                sg.Combo(
    ...
    def reset_info(self):
        ...
        self.window['video_small_category'].update('')
    ...
    def upload_video_info(self, video_info: dict):
        ...
        video_small_category = video_info.get('video_subcategory')

        video_info_list = [video_path, srt_path, video_big_category, video_small_category]
        if all(video_info_list):
            print('有效参数')
